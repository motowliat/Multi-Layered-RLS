function [H1_hat,e,e_f,Lambda,LAYERS] = FixedLambda_MultiLayered3(x1,d,N0,M1,delta,lambda_max,Max_Layers,Input_mode)
...
%     % Calculating the a posteriori error for second layer and after
        ...
        E_avg_f{c} = a*E_avg_f{c}+(1-a)*(abs(E_f{c})).^2;% obtaing the real-time power of E(l)
...
%     [Min,Layers] = min(Pi(1:Max_Layers)-2*(1-(1-lambda)*M1).^(1:Max_Layers)*N0);
