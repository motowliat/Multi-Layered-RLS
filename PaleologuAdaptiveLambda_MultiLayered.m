function [H1_hat,e,e_f,Lambda,LAYERS] = PaleologuAdaptiveLambda_MultiLayered(x1,d,N0,M1,delta,lambda_max,Max_Layers,Input_mode)
...
%     % Adaptive forgetting factor at RLS#>1
        if Lambda_Adaptivity == 1
            if l > 500 % use the Paleologu method after the initial error is setteled
               ...
               switch mode
                   case 1  % (1) Simple mode. lambda is calculated with Paleologu/Bhotto method
                            % for the first layer and it is the same for all
                            % layers
                        lambda_f{c} = lambda;

                   case 2  % (2) Complete mode. lambda is different at each layer 
            %                               and it is calculated based on Paleologu/Bhotto method             

                        % Paleologu Method
                        [lambda_f{c},sigma_e2_f{c},sigma_q2_f{c},sigma_v2_f{c}] = Adaptive_Forget_Factor_Paleologu...
...
% %   Power order checking
     if Input_mode == 3 % correlated
         ...
         Threshold = N0;%((1-(1-lambda)*M1)^2-(1-lambda)^2*M1)*N0
     end
     Layers = 1;
     for k = 2:Max_Layers
         if  (Pi(k) > Threshold) && (Pi(k-1) > Pi(k))
            Layers = k;
         else
            break;
         end
