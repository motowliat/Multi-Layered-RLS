...
function [H1_hat,e,e_f,Lambda,LAYERS] = Train_MultiLayered2(x1,d,N0,M1,delta,lambda_max,Layers_Adaptivity,Max_Layers)
...
%     % Adaptive number of layers
     ...
     if (E_avg > N0) && (Layers_Adaptivity == 1)
         Layers = max(2,Layers);
     elseif  (E_avg < N0) && (Layers_Adaptivity == 1)
         Layers = 1;
     end
...
if l >= 7000
    ...
    h1_hatImp = h1_f;
    r_hat_f(l) = d(l);
    for c = 1:Max_Layers % The second layer and after. The first SI cancellation is performed above, separately
...
%     % Adaptive number of layers
    if Layers > 1
        if (Layers_Adaptivity == 1) && (E_avg_f{Layers-1} > N0) && (E_avg_f{Layers} < N0)
           Layers = Layers;
        elseif (Layers_Adaptivity == 1) && (E_avg_f{Layers-1} < N0)
%            Layers = Layers-1;
        elseif (Layers_Adaptivity == 1) && (E_avg_f{Layers} > N0)
            Layers = min(Layers+1,Max_Layers);
        end
    end
