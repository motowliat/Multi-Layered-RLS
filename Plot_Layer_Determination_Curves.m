...
lambda_max = 1-.5/M1; % max forgetting factor for adaptive lambda
...
Input_mode = 2 ;%(1) BPSK, (2) AWGN, (3) AR Process
...
CHANNEL = 1; % number of channels to repeat the process
SNR = 20;% evaluating RT-SNR in (dB)
...
%             [H1_hat_3,e_3,Lambda_3] = PaleologuAdaptiveLambda_RLS(x1,y,N0,M1,delta,lambda_max); % simple RLS
...
% plot(SNR,Channel_Est_Error3_1);
plot(SNR,Channel_Est_Error3_2);
% plot(SNR,Channel_Est_Error3_3);
plot(SNR,Channel_Est_Error3_4);
plot(SNR,Channel_Est_Error3_5);
xlabel('SNR(dB)');
ylabel('MSE')
legend('Multi-Layered, Fixed-Lambda','RLS, Fixed-Lambda','LMS')
%     xlim([4800,6500])


subplot(3,1,3)
% plot(SNR,LA1_3); hold all
plot(SNR,LA2_3);
xlabel('SNR(dB)');
ylabel('L')
